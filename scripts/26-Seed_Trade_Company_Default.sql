...
==============================================================================
  ...
  编码：ANSI (GBK)
  说明：幂等可重复执行；演示账号口令由 sqlcmd -v AdminPassword 传入，且仅在首次插入时设置
==============================================================================
*/
SET NOCOUNT ON;
SET XACT_ABORT ON;
...
/* 演示账号口令不写在脚本里，用 sqlcmd 变量传入：
       sqlcmd -S <server> -d <db> -v AdminPassword="你的强口令" -i 26-Seed_Trade_Company_Default.sql
...
BEGIN
    RAISERROR(N'请用 -v AdminPassword="..." 传入演示账号初始口令后再执行本脚本。', 16, 1);
    SET NOEXEC ON;
END
...
/* ----- 演示用户（口令取自 -v AdminPassword，仅首次插入时设置） ----- */
...
ELSE
    UPDATE dbo.Tbl_E_Users SET RealName=N'系统管理员', IsEnabled=1, BStatus='1', IsDeleted=0, AmendDate=@Now, Operator=@Op
...
ELSE
    UPDATE dbo.Tbl_E_Users SET RealName=N'张总', IsEnabled=1, BStatus='1', IsDeleted=0, AmendDate=@Now, Operator=@Op
...
ELSE
    UPDATE dbo.Tbl_E_Users SET RealName=N'李销售经理', IsEnabled=1, BStatus='1', IsDeleted=0, AmendDate=@Now, Operator=@Op
...
ELSE
    UPDATE dbo.Tbl_E_Users SET RealName=N'王销售', IsEnabled=1, BStatus='1', IsDeleted=0, AmendDate=@Now, Operator=@Op
...
ELSE
    UPDATE dbo.Tbl_E_Users SET RealName=N'赵采购经理', IsEnabled=1, BStatus='1', IsDeleted=0, AmendDate=@Now, Operator=@Op
...
ELSE
    UPDATE dbo.Tbl_E_Users SET RealName=N'钱采购', IsEnabled=1, BStatus='1', IsDeleted=0, AmendDate=@Now, Operator=@Op
...
ELSE
    UPDATE dbo.Tbl_E_Users SET RealName=N'孙仓储主管', IsEnabled=1, BStatus='1', IsDeleted=0, AmendDate=@Now, Operator=@Op
...
ELSE
    UPDATE dbo.Tbl_E_Users SET RealName=N'周财务经理', IsEnabled=1, BStatus='1', IsDeleted=0, AmendDate=@Now, Operator=@Op
...
ELSE
    UPDATE dbo.Tbl_E_Users SET RealName=N'吴人事主管', IsEnabled=1, BStatus='1', IsDeleted=0, AmendDate=@Now, Operator=@Op
