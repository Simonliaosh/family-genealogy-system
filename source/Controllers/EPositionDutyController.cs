...
[Authorize]
public class EPositionDutyController : Controller
{
    private readonly EPositionDutyService _service;

    private static readonly HashSet<string> SearchWhitelist = new(StringComparer.OrdinalIgnoreCase)
    {
        "Position", "Duty", "BusinessLimit", "Remark"
    };

    public EPositionDutyController(EPositionDutyService service)
    {
        _service = service;
    }
        ...
        CancellationToken ct = default)
    {
        ...
        var sf = SearchWhitelist.Contains((searchField ?? "").Trim()) ? (searchField ?? "").Trim() : "Position";
        ...
        ViewBag.BStatusOptions = EPositionDutyService.BuildBStatusFilterOptions();
        ViewBag.SearchFieldOptions = EPositionDutyService.BuildSearchFieldOptions();
    ...
    public async Task<IActionResult> Create(CancellationToken ct = default)
    {
        ...
        return View(new EPositionDutyFormVm());
    }

    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Create(EPositionDutyFormVm model, string? selectNo, CancellationToken ct = default)
    {
        ...
        {
            ModelState.AddModelError(nameof(model.DutyId), ex.Message);
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Edit(int id, EPositionDutyFormVm model, string? selectNo, string? polistRt, CancellationToken ct = default)
    {
        if (string.Equals(selectNo, "9", StringComparison.OrdinalIgnoreCase))
    ...
    private async Task<IActionResult> ReturnFormInvalid(string viewName, EPositionDutyFormVm model, CancellationToken ct)
    {
        await PopulateFormOptionsAsync(ct);
    ...
    {
        var pos = await _service.LoadActivePositionsAsync(ct);
        var duty = await _service.LoadActiveDutiesAsync(ct);
        var dept = await _service.LoadActiveDepartmentsAsync(ct);

        ViewBag.PosOptions = pos.Select(x => (x.Id.ToString(), $"{x.Code} - {x.Name}".Trim())).ToList();
        ViewBag.DutyOptions = duty.Select(x => (x.Id.ToString(), $"{x.Code} - {x.Name}".Trim())).ToList();
        var deptOptions = new List<(string Value, string Text)> { ("", "不限") };
        deptOptions.AddRange(dept.Select(x => (x.Id.ToString(), $"{x.Code} - {x.Name}".Trim())));
        ViewBag.DeptOptions = deptOptions;
        ViewBag.BStatusFormOptions = EPositionDutyService.BuildBStatusFormOptions();
    }
