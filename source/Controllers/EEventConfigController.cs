...
[Authorize]
public class EEventConfigController : Controller
{
    private readonly EEventConfigService _service;

    private static readonly HashSet<string> SearchWhitelist = new(StringComparer.OrdinalIgnoreCase)
    {
        "AppCode", "EventCode", "EventName", "PageUrl", "Remark"
    };

    public EEventConfigController(EEventConfigService service)
    {
        _service = service;
    }
        ...
        CancellationToken ct = default)
    {
        ...
        var (pageRows, total, totalPages, pageOut) = await _service.GetIndexPageAsync(
            bStatus1, execType1, menuGroupCode1, sf, sc, sortField, sortArrow, page, pSize, ct);
        ...
        ViewBag.BStatusOptions = EEventConfigService.BuildBStatusFilterOptions();
        ViewBag.ExecTypeOptions = await _service.GetExecTypeFilterOptionsAsync(ct);
        ViewBag.MenuGroupFilterOptions = await _service.BuildMenuGroupFilterOptionsAsync(ct);
        ViewBag.SearchFieldOptions = EEventConfigService.BuildSearchFieldOptions();
    ...
    public async Task<IActionResult> Create(CancellationToken ct = default)
    {
        ...
        return View(new EEventConfigFormVm());
    }

    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Create(EEventConfigFormVm model, string? selectNo, CancellationToken ct = default)
    {
        ...
        foreach (var (key, message) in await _service.GetSaveValidationErrorsAsync(model, false, ct))
        ...
        {
            ModelState.AddModelError(nameof(model.EventCode), ex.Message);
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Edit(int id, EEventConfigFormVm model, string? selectNo, string? polistRt, CancellationToken ct = default)
    {
        if (string.Equals(selectNo, "9", StringComparison.OrdinalIgnoreCase))
        ...
        model.EventCode = row.EventCode;
        _service.NormalizeFormForSave(model);
        if (!ModelState.IsValid) return await ReturnEditInvalidAsync(model, ct);

        foreach (var (key, message) in await _service.GetSaveValidationErrorsAsync(model, true, ct))
            ModelState.AddModelError(key, message);
        if (!ModelState.IsValid) return await ReturnEditInvalidAsync(model, ct);
    ...
    private async Task<IActionResult> ReturnCreateInvalidAsync(EEventConfigFormVm model, CancellationToken ct)
    {
        await PopulateFormOptionsAsync(model, ct);
        return View(nameof(Create), model);
    }

    private async Task<IActionResult> ReturnEditInvalidAsync(EEventConfigFormVm model, CancellationToken ct)
    {
        ViewBag.EditMode = true;
        await PopulateFormOptionsAsync(model, ct);
    ...
    private async Task PopulateFormOptionsAsync(EEventConfigFormVm? model, CancellationToken ct)
    {
        ViewBag.AppCodeOptions = EEventConfigService.BuildAppCodeFormOptions();
        ViewBag.ExecTypeOptions = await _service.GetExecTypeFormOptionsAsync(ct);
        ViewBag.EventTypeOptions = await _service.GetEventTypeFormOptionsAsync(ct);
        ViewBag.HandleModeOptions = await _service.GetHandleModeFormOptionsAsync(ct);
        ViewBag.BStatusOptions = EEventConfigService.BuildBStatusFormOptions();
