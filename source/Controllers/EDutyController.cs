...
[Authorize]
public class EDutyController : Controller
{
    private readonly EDutyService _service;

    private static readonly HashSet<string> SearchWhitelist = new(StringComparer.OrdinalIgnoreCase)
    {
        "DutyCode", "DutyCName", "DutyEName", "Remark", "DDescription", "DutyFlow"
    };

    private static readonly HashSet<string> SortWhitelist = new(StringComparer.OrdinalIgnoreCase)
    {
        "DutyCode", "DutyCName", "DutyEName", "DutyDispSeq", "BStatus"
    };

    public EDutyController(EDutyService service)
    {
        _service = service;
    }
        ...
        CancellationToken ct = default)
    {
        ...
        var sf = SearchWhitelist.Contains((searchField ?? "").Trim()) ? (searchField ?? "").Trim() : "DutyCode";
        ...
        ViewBag.BStatusOptions = EDutyService.BuildBStatusFilterOptions();
    ...
    public async Task<IActionResult> Create(CancellationToken ct = default)
    {
        ...
        return View(new EDutyFormVm());
    }

    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Create(EDutyFormVm model, string? selectNo, CancellationToken ct = default)
    {
        ...
        {
            ModelState.AddModelError(nameof(model.DutyCode), ex.Message);
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Edit(int id, EDutyFormVm model, string? selectNo, string? polistRt, CancellationToken ct = default)
    {
        if (string.Equals(selectNo, "9", StringComparison.OrdinalIgnoreCase))
    ...
    {
        ViewBag.BStatusOptions = EDutyService.BuildBStatusFormOptions();
        ViewBag.DutyCategoryOptions = await _service.GetDutyCategoryFormOptionsAsync(ct);
    }

    private async Task<IActionResult> ReturnCreateInvalidAsync(EDutyFormVm model, CancellationToken ct)
    {
        await SetFormOptionsAsync(ct);
        return View(nameof(Create), model);
    }

    private async Task<IActionResult> ReturnEditInvalidAsync(EDutyFormVm model, CancellationToken ct)
    {
        ViewBag.EditMode = true;
        await SetFormOptionsAsync(ct);
