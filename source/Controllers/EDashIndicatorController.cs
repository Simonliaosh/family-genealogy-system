...
[Authorize]
public class EDashIndicatorController : Controller
{
    private readonly DashIndicatorService _service;
    ...
    public EDashIndicatorController(DashIndicatorService service)
    {
        _service = service;
    }
        ...
        CancellationToken ct = default)
    {
        ...
        var sf = SearchWhitelist.Contains((searchField ?? "").Trim()) ? (searchField ?? "").Trim() : "IndicatorCode";
        ...
        ViewBag.BStatusOptions = DashIndicatorService.BuildBStatusFilterOptions();
    ...
    public async Task<IActionResult> Create(CancellationToken ct = default)
    {
        ...
        return View(new DashIndicatorFormVm());
    }

    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Create(DashIndicatorFormVm model, string? selectNo, CancellationToken ct = default)
    {
        ...
        {
            ModelState.AddModelError(nameof(model.IndicatorCode), ex.Message);
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Edit(int id, DashIndicatorFormVm model, string? selectNo, string? polistRt, CancellationToken ct = default)
    {
        if (string.Equals(selectNo, "9", StringComparison.OrdinalIgnoreCase))
    ...
    {
        ViewBag.ChartTypeOptions = DashIndicatorService.BuildChartTypeFormOptions();
        ViewBag.BStatusOptions = DashIndicatorService.BuildBStatusFormOptions();
        ViewBag.AppCodeOptions = await _service.GetAppCodeFormOptionsAsync(ct);
    }

    private async Task<IActionResult> ReturnCreateInvalidAsync(DashIndicatorFormVm model, CancellationToken ct)
    {
        await SetFormOptionsAsync(ct);
        return View(nameof(Create), model);
    }

    private async Task<IActionResult> ReturnEditInvalidAsync(DashIndicatorFormVm model, CancellationToken ct)
    {
        ViewBag.EditMode = true;
        await SetFormOptionsAsync(ct);
